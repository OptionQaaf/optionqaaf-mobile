...
export default function OrdersScreen() {
  ...
  return (
    <AuthGate requireAuth fallback={<AccountSignInFallback onSuccess={() => router.replace("/account/orders" as const)} />}>
      <Screen bleedBottom>
        <MenuBar back />
        <OrdersContent />
      </Screen>
...
function OrdersContent() {
          ...
          <Card padding="lg" className="gap-4">
            <View className="flex-row items-start justify-between gap-3">
              ...
              <View className="items-end gap-1">
                <Badge label={formatStatus(item.latestFulfillmentStatus)} tone="accent" />
              </View>
            </View>
            <View className="gap-2">
              ...
              {remaining > 0 ? (
                <Text className="text-[#64748b] text-[12px]">+{remaining} more item{remaining === 1 ? "" : "s"}</Text>
              ) : null}
            </View>
  ...
  return (
    <FlatList
      ...
      refreshControl={<RefreshControl refreshing={isRefetching || isLoading} onRefresh={() => refetch()} tintColor="#111827" />}
