...
import {
  CartCreateDocument, type CartCreateMutation,
  CartLinesAddDocument, type CartLinesAddMutation,
  CartQueryDocument, type CartQueryQuery,
  ProductByHandleDocument, type ProductByHandleQuery,
  ...
  const productQ = useQuery({
    ...
    queryFn: async () =>
      shopifyClient.request<ProductByHandleQuery>(ProductByHandleDocument, {
        handle: handle!, country: locale.country as any, language: locale.language as any
      }),
  });
  ...
  const ensureCart = useMutation({
    mutationFn: async () => {
      ...
      const res = await shopifyClient.request<CartCreateMutation>(CartCreateDocument, {
        country: locale.country as any, language: locale.language as any,
      });
  ...
  const cartQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await shopifyClient.request<CartQueryQuery>(CartQueryDocument, {
        id: cartId, country: locale.country as any, language: locale.language as any
      });
      return res.cart;
    },
  });
